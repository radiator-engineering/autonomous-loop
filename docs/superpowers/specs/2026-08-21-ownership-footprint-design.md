...
**3. Finalize reconciles**: one instruction added to the finalize prompt — compare the working tree's changed files (`git status --porcelain`; the run's base commit is not something finalize can know) against the union of claimed files in `footprint.jsonl`; list edits no attempt claimed, and files claimed by two different items, as one line each under "Traps". Surface, don't gate: reconciliation changes no status and touches no schema. Honest limit, as it stood in v1: work a run committed along the way escapes this comparison. **Superseded by subtask 2** (spec: `2026-08-21-verified-commits-design.md`), not contradicted: once every verified pass commits the round it happens, escaping this comparison is *correct* rather than a gap — committed work is accounted for at HEAD, and the diff this reconciliation reads is left holding only genuine, unverified residue. The leftover hazard still lives in the uncommitted tree; subtask 2 just shrinks what can legitimately be sitting there.
...
- **No janitor.** Mechanical cleanup stays subtask 2 of #8; this spec supplies the record that will make it safe. **Delivered**: subtask 2 (spec: `2026-08-21-verified-commits-design.md`) does not add a separate janitor agent or process, but it does make the retry worker's OWN cleanup of its OWN claimed files mechanical rather than evaluative — commit-per-verified-pass means nothing verified is ever left uncommitted, so `retryDirective` now resets those files unconditionally instead of asking the worker to judge each leftover.
