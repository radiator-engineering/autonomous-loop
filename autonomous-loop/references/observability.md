...
**Commit-per-verified-pass** (issue #8 subtask 2, spec: `2026-08-21-verified-commits-design.md`). With `VERIFIED_COMMITS` on (the default), the Ledger step — one agent, run once per round, after every verifier in the round's batch has answered — commits each item that verified `pass` this round: it reads `footprint.jsonl` for that item's claimed files, `git add -A -- <those files>` (never a bare `git add -A`, which would sweep in a *different* item's still-uncommitted work from the same `BATCH>1` round), and `git commit -m "<id>: …"` — one commit per item, never a combined commit for the round. This is why the Ledger step, not the Verify step, does it: verify runs in parallel (up to `BATCH` items), and a `git commit` racing another verifier's for the same `.git/index` would silently lose a genuine pass. The canonical artifact becomes literally the last commit on the branch, and `retryDirective` changes to match: instead of asking the retry worker to read a diff and judge each leftover (subtask 1's text), it now resets the item's own claimed files unconditionally — `git checkout` / `git clean` — because nothing verified can ever be sitting uncommitted any more; anything still in the diff is, by construction, the dead attempt's own residue. `VERIFIED_COMMITS` defaults true and is a no-op (with a note in the Ledger agent's report, not a failure) on a target that is not a git working tree.
