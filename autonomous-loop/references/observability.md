...
reported as merged, because there is nothing to land and nothing to retry. A clean merge lands the item; a genuine conflict is a NEW failure mode, reported structurally (`MERGE_SCHEMA`, not free text) and treated as an OPEN MANDATE — filed under `unverified`, exactly where a crashed verifier's item is filed, so it is not confirmed, not counted, holds `gap` true (no positive terminal status can fire while it is open), and is retried into a fresh worktree off the (by then updated) mainline, which `worktreeDirective`'s unconditional reset already provides with no separate instruction needed. `retryDirective` shrinks to a short constant note: there is nothing left on the shared tree to reset, because a dead attempt never touched it. The Coherence pass (only the converger archetype implements it) relocates from before Verify to after Merge, reconciling only the items that actually landed — before Merge runs, the shared tree has not changed at all this round, so there is nothing yet to reconcile. It is the one step that edits the shared tree directly, so it COMMITS what it changes — staging only the files it edited, by name (`git add -- <files>`, never a blanket add: the operator may have unrelated work in the tree, which is reason enough on its own and holds in every configuration; and where `LEDGER_DIR` sits inside the tree with no `.gitignore` of its own, a blanket add additionally stages the run's own bookkeeping and commits each attempt worktree under it as a gitlink pointing at a branch that may not outlive the run. That second hazard is narrower than it once read here. `scripts/workbench_server.py` writes a `.gitignore` containing `*` into the run dir at startup, and the documented launch order starts the workbench first, so on the default path the ledger dir is already ignored. Measured in a scratch repo with `LEDGER_DIR` inside the target and an attempt worktree under it: with that `.gitignore` present, `git add -A` stages nothing at all and `git status --porcelain` stays empty; delete it and the same command stages `activity.jsonl` and the worktree at mode `160000`, warning `adding embedded git repository`. The gitlink half therefore describes a run dir created without the workbench, or predating the version that writes the file — the operator-work-in-progress half needs no such condition, and either half alone settles the rule) — and finishes with none of *its own* edits uncommitted: nothing else in the run commits that tree, and an uncommitted edit left behind is not merely lost from HEAD — it blocks the next round's merge of any attempt touching the same file. It is told NOT to aim for an empty `git status`, and never to stash, checkout, reset or clean to reach one. That state is unreachable by any permitted action — `LEDGER_DIR` and any operator work-in-progress both show up in `git status` and are supposed to — so demanding it would leave the agent one correct-sounding step from destroying work that is not its own. The Merge prompt carries the matching rule for the case where a dirty shared tree makes `git merge` refuse to start (exit 2, no `MERGE_HEAD`, empty `--diff-filter=U`, `--abort` fails): that is reported as a conflict, never forced through. `VERIFIED_COMMITS` defaults true and MUST be set false on a target that is not a git working tree: it is not self-disabling. The worker and verifier directives gate on the constant alone, so a `true` on a non-git target hands every worker a `git worktree add` that cannot run; only the Merge phase probes the tree itself (`git rev-parse --is-inside-work-tree`), and it never runs when nothing verified pass.
